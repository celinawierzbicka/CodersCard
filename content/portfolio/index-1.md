---
portfolioPageTitle: Portfolio
portfolioPageDescription: null
portfolioPageImage: null
projects:
  - projectPreviewNote: Create, customize, and publish your online resume in minutes
    projectTechnologies:
      - technologyName: React
      - technologyName: Gatsby
      - technologyName: TypeScript
      - technologyName: Material-UI
      - technologyName: Netlify CMS
    projectDescription: Template for easy setup of resume using Netlify CMS.
    projectName: CodersCard
    projectCode: https://github.com/CodersCrew/CodersCard
    projectApp: ""
    projectPreviewImage: coders-card.png
    projectRole: Developing components and page views.
    projectMockups: ""
    projectStartDate: 2020-07-01T10:45:41.756Z
    projectFinishDate: 2020-11-30T11:45:41.772Z
    projectLabel: Web app
  - projectPreviewNote: Helping kids to grow healthy
    projectTechnologies:
      - technologyName: React
      - technologyName: TypeScript
      - technologyName: Material-UI
      - technologyName: GraphQL
    projectDescription: "Web app for[ Mali Wspaniali
      Organization](http://mali-wspaniali.pl/) used for the management and
      tracking of all activities including: physical activity test result and
      progress tracking, management of kindergartens, kind and instructors."
    projectName: Mali Wspaniali
    projectCode: ""
    projectApp: ""
    projectPreviewImage: mali-wspaniali.jpg
    projectRole: Working on new page views and functionalites, fixing bugs, code
      review of other team members work.
    projectMockups: ""
    projectStartDate: 2020-01-01T11:45:41.756Z
    projectFinishDate: 2021-03-31T09:45:41.772Z
    projectLabel: Web app
---
